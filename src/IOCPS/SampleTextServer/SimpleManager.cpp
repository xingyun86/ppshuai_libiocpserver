...
//|                                                                  |
//+------------------------------------------------------------------+
int simple_wmain(int argc,wchar_t* argv[])
  {
...
//--- loading manager API
   if((res=factory.Initialize(L".\\"))!=MT_RET_OK)
     {
...
//--- return
  ...
  ////////////////////////////////////////////////////////////////////////////////////////
  //--- order events
  void CMTTickInstance::OnOrderAdd(const IMTOrder*    order) 
  {
  }
  void CMTTickInstance::OnOrderUpdate(const IMTOrder* order) 
  {
  }
  void CMTTickInstance::OnOrderDelete(const IMTOrder* order) 
  {
  }
  void CMTTickInstance::OnOrderClean(const UINT64 login) 
  { 
  }
  //--- tick events
  void CMTTickInstance::OnTick(LPCWSTR symbol, const MTTickShort& tick)
  { 
	  char szData[USHRT_MAX + 1] = { 0 };
	  //sprintf(szKey, "%ws", symbol);
	  sprintf(szData, "{\"symbol\":\"%ws\",\"time\":\"%lld\",\"ask\":\"%lf\",\"bid\":\"%lf\",\"flags\":\"%I64u\",\"vol\":\"%lf\"}", symbol, tick.datetime, tick.ask, tick.bid, tick.flags, SMTMath::VolumeToDouble(tick.volume));
	  //sync_test(szKey, szData);
	  m_redis_publisher->publish(szData);

	  this->getDequeLock()->lock();
	  this->getDequeList()->push_back(szData);
	  this->getDequeLock()->unlock();
  }
  void CMTTickInstance::OnTickStat(const MTTickStat& /*tick*/) 
  {
